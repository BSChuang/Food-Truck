...
CREATE TABLE cs4400spring2020.Orders (
	orderID INT(10) ZEROFILL AUTO_INCREMENT PRIMARY KEY,
    date DATE NOT NULL,
...
BEGIN
	IF (((i_email IS NOT NULL AND i_email != '') OR (i_balance IS NOT NULL AND i_balance > 0))
			AND CHAR_LENGTH(i_password) >= 8) THEN
		INSERT INTO `User` (username, `password`, firstName, lastName)
		VALUES (i_username, md5(i_password), i_firstname, i_lastname);

		IF (i_email IS NOT NULL AND i_email != '') THEN
			IF (i_type in ('Admin', 'Manager','Staff')) THEN
			...
			IF (i_type = 'Admin') THEN
					...
					VALUES (i_username);
			ELSEIF (i_type = 'Manager') THEN
					INSERT INTO manager (username)
					VALUES (i_username);
			ELSEIF (i_type = 'Staff') THEN
...
BEGIN
	...
	SELECT FoodTruck.foodTruckName, Station.stationName, (station.capacity - occupied) as remainingCapacity, COUNT(DISTINCT username) as staffCount, COUNT(DISTINCT foodName) as menuItemCount
    FROM FOODTRUCK
    INNER JOIN STATION
    ...
    on a.stationName = FOODTRUCK.stationName
    WHERE
    (i_managerUsername = FoodTruck.managerUsername OR i_managerUsername is NULL OR i_managerUsername = '') AND
    (foodtruck.foodTruckName LIKE CONCAT('%', i_foodTruckName, '%') OR i_foodTruckName IS NULL OR i_foodTruckName = '') AND
    (i_stationName = Station.stationName OR i_stationName IS NULL OR i_stationName = '') AND
    ((i_hasRemainingCapacity = TRUE AND (station.capacity-occupied)>0) OR (i_hasRemainingCapacity = FALSE))
...
BEGIN
   ...
   DELETE FROM FOODTRUCK
	WHERE i_foodTruckName = foodTruck.foodTruckName;
...
-- user must already exist as an employee, but not be assigned to any food truck
...
-- deleted underscore, food must already exist 
...
-- CHANGES deleted underscores, changed function significantly CHECK AGAINST DOCUMENT
...
BEGIN
	...
	DROP TABLE IF EXISTS mn_view_foodTruck_available_staff_result;
     CREATE TABLE mn_view_foodTruck_available_staff_result(availableStaff varchar(100))
		...
		WHERE
        staff.foodTruckName IS NULL OR #staff members who are not assigned to any truck
        staff.foodtruckName = i_foodTruckName; #staff members already assigned to this truck
...
-- deleted underscores and removed ambiguity
...
-- deleted underscores, fixed ambiguity
...
BEGIN
    ...
    (   SELECT FoodTruck.foodTruckName as foodTruckName, COUNT(DISTINCT (Orders.orderID)) as totalOrder, SUM(MenuItem.price * OrderDetail.purchaseQuantity) as totalRevenue, COUNT(DISTINCT customerUsername) as totalCustomer
        ...
        (i_stationName IS NULL OR i_stationName = '' OR FoodTruck.stationName = i_stationName) AND
...
-- changes made, final version tested
...
BEGIN
    ...
    SELECT Station.stationName, Station.buildingName, GROUP_CONCAT(DISTINCT(FoodTruck.foodTruckName) SEPARATOR ',') AS foodTruckNames, GROUP_CONCAT(DISTINCT(menuitem.foodName) SEPARATOR ',') AS foodNames
        ...
        (i_buildingTag is NULL OR i_buildingTag = '' OR BuildingTag.tag LIKE CONCAT('%', i_buildingTag, '%')) 
		GROUP BY Station.stationName
        HAVING (i_foodname is NULL OR foodNames like CONCAT('%', i_foodname, '%')) AND # these two conditions were moved out here so they can check the aggregate columns
        (i_foodTruckName is NULL OR foodTruckNames LIKE CONCAT('%', i_foodTruckName, '%'))
        ;
...
BEGIN
	...
	SELECT b.stationName, b.buildingName, group_concat(tag SEPARATOR ',') AS "tags", c.description, b.balance 
	FROM buildingTag a 
	NATURAL JOIN (SELECT a.*, b.buildingName 
		FROM customer a 
		NATURAL JOIN station b 
		WHERE username = i_customerUsername) b
	NATURAL JOIN building c;
...
BEGIN
    ...
    SELECT foodTruckName, concat(firstName, " ", lastName) as name, GROUP_CONCAT(C.foodName SEPARATOR ",") as menu
	FROM foodtruck A
	NATURAL JOIN
	(SELECT a.username, b.stationName
			FROM customer a 
			NATURAL JOIN station b 
			WHERE username = i_customerUsername) B
	NATURAL JOIN menuitem C
	JOIN user D
	ON managerUsername = D.username
	GROUP BY foodTruckName;
...
BEGIN
    ...
    # find price of desired order
    select (menuitem.price * i_purchaseQuantity) as price
    into @price
    from menuitem
    where menuitem.foodname = i_foodname
    and menuitem.foodtruckname = i_foodtruckname;

	# find customer balance
	select customer.balance, customer.username, customer.stationName
    into @bal, @cName, @sName
    from customer
	inner join orders on customer.username = orders.customerUsername
	where orders.orderid = i_orderID;
    
    #compare these two in if then statement to determine validity of order
    IF (@bal > @price) THEN
		UPDATE customer
...
BEGIN
    ...
    SELECT date, c.orderID, sum(purchaseQuantity * price) as orderTotal, group_concat(distinct a.foodName SEPARATOR ",") as foods, sum(purchaseQuantity) as foodQuantity
	FROM orders c
	LEFT JOIN orderdetail a
    ON c.orderID = a.orderID
	LEFT JOIN menuitem b
	ON a.foodTruckName = b.foodTruckName and a.foodName = b.foodName
